<fieldset>
    ...
    <table cellpadding="2" cellspacing="1" border="0" class="form">
        <tr>
            ...
            <td><?php $objEditItem->qText('title', '', 'width:50px'); ?>
                <small>Mr. Mrs. Ms. Dr. Pr. etc...</small>
            </td>
        </tr>
        <tr>
...
<fieldset>
        ...
        if ($objEditItem->isLoaded()) {
            ...
            <tr>
                <td colspan="2" align="center">
                    <small><?php echo $langUser['password_legend']; ?></small>
                </td>
            </tr>
            <?php
        }
    ...
    if ($objUser->checkLevel(2) && ($objUser->id != $objEditItem->id)) {
        ...
        ?>
        <p>
            <span><?php $objEditItem->qCheckBox('enabled', $objEditItem->enabled, '', 'onClick="tE(gE(\'cust1\'))"'); ?>
                <label><?php echo $langUser['enabled_label']; ?></label></span>
            <span id="cust1"
                  style="display:<?php echo ($objEditItem->enabled) ? 'inline' : 'none'; ?>"><?php $objPositionList->qSelect('level', $objEditItem->level); ?></span>
        </p>
        <?php
    }
    ?>
