...
$objUserProjectList->addOrder('projectStatus_statusKey ASC, name ASC');
...
<table cellpadding="2" cellspacing="0" border="0">
    ...
    <tr>
        ...
        <td<?php echo (@constant('FRK_CONTEXT_ENABLE')) ? ' colspan="3"' : ''; ?>>
            <input type="text" name="deadlineDate" class="dead" id="freak_cal_d"
                   value="<?php echo ($objTask->deadlineDate) ? $objTask->getDte('deadlineDate', 'FRM') : ''; ?>"/>
            <img src="skins/<?php echo FRK_SKIN_FOLDER; ?>/images/cal.gif" id="freak_cal_b" style="cursor: pointer;"
                 alt="Calendar" title="Date selector"/>
    ...
    <tr>
        ...
        <td<?php echo (@constant('FRK_CONTEXT_ENABLE')) ? ' colspan="3"' : ''; ?>>
			<span id="project_sel" style="display:block"><?php
                ...
                ?> <a href="javascript:freak_switch_protxt()"
                      class="small">&gt; <?php echo $GLOBALS['langForm']['project_new']; ?></a>
			</span>
            <span id="project_txt" style="display:none"><input type="text" name="project2" value=""
                                                               style="width:200px"/>
			  <a href="javascript:freak_switch_prosel()"
                 class="small">&lt; <?php echo $GLOBALS['langForm']['project_list']; ?></a>
                <?php
                }
                ?>
            </span>
    ...
    <tr>
        ...
        <td<?php echo (@constant('FRK_CONTEXT_ENABLE')) ? ' colspan="3"' : ''; ?>><input type="text" name="title"
                                                                                         value="<?php echo ($objTask->title) ? $objTask->getValue('title') : ''; ?>"
                                                                                         class="full"/></td>
    </tr>
    <?php if (@constant('FRK_DESCRIPTION_ENABLE')) { ?>
        <tr valign="top">
            <th><?php echo $GLOBALS['langForm']['description']; ?>:</th>
            <td<?php echo (@constant('FRK_CONTEXT_ENABLE')) ? ' colspan="3"' : ''; ?>><textarea name="description"
                                                                                                class="full"><?php echo $objTask->description; ?></textarea>
            </td>
        </tr>
    <?php } ?>
    <tr>
        ...
        <td<?php echo (@constant('FRK_CONTEXT_ENABLE')) ? ' colspan="3"' : ''; ?>>
            ...
            ?>
            <span><input type="radio" name="showPrivate"
                         value="0" <?php if ($objTask->showPrivate == 0) echo 'checked="checked" '; ?>/><label><?php echo $GLOBALS['langForm']['public']; ?></label></span>
            <?php
            if ($objUser->checkLevel(12)) {
                ?>
                <span><input type="radio" name="showPrivate"
                             value="1" <?php if ($objTask->showPrivate == 1) echo 'checked="checked" '; ?>/><label><?php echo $GLOBALS['langForm']['internal']; ?></label></span>
                <?php
            ...
            ?>
            <span><input type="radio" name="showPrivate"
                         value="2" <?php if ($objTask->showPrivate == 2) echo 'checked="checked" '; ?>/><label><?php echo $GLOBALS['langForm']['private']; ?></label></span>
        </td>
    </tr>
    <tr>
